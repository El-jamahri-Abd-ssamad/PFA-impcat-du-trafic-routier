...
def load_and_clean_data(file_path, output_cleaned_file, chunksize=50000):
    """
    ...
    Args:
        ...
        output_cleaned_file (str): Chemin du fichier nettoyé temporaire.
    ...
    # Étape 2 : Supprimer les doubles quotes dans le fichier
    try:
        ...
        with open(output_cleaned_file, 'w', encoding='utf-8') as fichier_sortie:
            fichier_sortie.write(contenu_modifie)
        
        print(f"Les doubles quotes ont été supprimées. Fichier nettoyé enregistré dans {output_cleaned_file}")
    ...
    # Étape 3 : Charger les données nettoyées en morceaux
    try:
        chunks = pd.read_csv(output_cleaned_file, delimiter=';', chunksize=chunksize)
...
def main():
    ...
    output_cleaned_file = "data_frame_cleaned.csv"
    output_file = "data_cleaned.csv"

    # Chargement du fichier en morceaux
    chunks = load_and_clean_data(input_file, output_cleaned_file)
    if chunks is None:
        return
